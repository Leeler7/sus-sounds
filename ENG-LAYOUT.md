...
- [x] **T0a (P0, SPIKE FIRST) — PASSED (cross-arch confirmed 2026-06-17).** CI on github.com/Leeler7/sus-sounds: Windows x64, macOS arm64 (Apple Silicon), and the local Windows build ALL produced the bit-identical hash `88bfd0f0...e537d48c`. That's a match across OS AND CPU architecture (x86-64 vs ARM64) — the hardest determinism case. Box2D v3.1.1 cross-platform determinism holds; the shared-board promise is real. Output is raw IEEE-754 bit patterns (not %a). PENDING: macOS Intel (macos-13) runner was still queued at report time — same x64 arch as Windows (already matched), so low-risk; the compare job formally closes the gate when it lands. Linux render FAILED to build (descoped; cause TBD, see BACKLOG). Permanent regression guard: `.github/workflows/determinism.yml`.
...
- [x] **T1 (P1) — DONE (2026-06-17).** Built `core/` (JUCE-free): `PhysicsCore.{h,cpp}` (Box2D v3.1.1, fixed 1 kHz sim step advanced by a round-to-nearest TIME accumulator → sample-rate-independent + block-size-invariant), `Rng.h` (portable PCG32, no std:: distributions), `tests/determinism_test.cpp`, `CMakeLists.txt` (no -ffast-math/FMA). All 6 checks GREEN on Windows: determinism, block-size invariance, **sample-rate invariance (44.1/48/96k bit-identical)**, RNG determinism + sensitivity, liveliness (ball cascades ~17 hits/loop, loops). No-stuck: sustained-slow detection + small mass-scaled horizontal nudge + exit/timeout respawn. Symmetry-breaking off-center drop + initial vx (a perfectly symmetric drop balances on the apex peg). FOLLOW-UP (by ear, not blocking): cascade richness — a near-centered landing can still balance until the nudge frees it; tune the board feel once audio is wired (BACKLOG). NEXT: wire core into the JUCE processor + the audio engine (PegToTap/DelayEngine).
  - Per-peg **bumper** added (BoardParams.pegRest[]): >1.0 returns extra energy (pinball
    bumper). Deterministic; verified in the test. Audio side should accent bumper hits.
...
- [~] **T7 (P2) — IN PROGRESS (2026-06-17).** Sound engine per AGREED model (ARCHITECTURE §5.2). DONE: `core/SoundEngine.{h,cpp}` (exciter voices + tempo-synced stereo feedback delay + Schroeder reverb), `pegToTap` mapping (x→pan, y→scale pitch, energy→level+brightness), `Wav.h`, `tools/render_demo` (physics→engine→WAV). Peg type plumbed through physics (BoardParams.pegType + Collision.type via shape userData). Renders audible stereo; delay + reverb pegs both fire, energy/bumper accents work, determinism still green. INPUT-AUDIO PATH DONE: `Wav.h` reader (16/24/32-bit + float32), `SoundEngine::setInput`, input-grain voices (the ball echoes/reverbs grains of YOUR audio over a dry passthrough), per-grain one-pole lowpass for brightness/darkness. Verified on a 24-bit/48k stereo file. REMAINING: by-ear feel tuning (cascade still sparse); FTZ/DAZ denormal guard; then JUCE wiring + GUI. Params already exposed for the eventual GUI: delay time/feedback (timing/decay), grain lowpass (bright/dark), reverb decay/mix (depth/size), pan width, pitch range.
...
- [~] **T10 / JUCE SHELL — first GUI DONE (2026-06-17).** `plugin/` JUCE 8.0.13 plugin (VST3 + Standalone), reuses `plinko_core`. PluginProcessor wires physics + sound engine into processBlock (APVTS macros: Gravity, Feedback, Delay, Reverb, Size, Tone, Width, Level — all live), gravity live via b2World_SetGravity. PluginEditor: animated BoardComponent (60fps; pegs amber=delay/teal=reverb/red=bumper, white ball) + 8 rotary knobs. Builds clean; determinism still green. RUNS in **exciter mode** (standalone instrument). **INTERACTIVE BOARD DONE 2026-06-17:** click=add peg, drag=move, right-click=delete, double-click=toggle delay/reverb; edits commit to the processor (try-lock) and re-init the live physics (re-init allocates on the audio thread — acceptable per-edit cost; lock-free body-pool path still backlogged). Pegs shrunk (radius 0.022 < ball 0.03) — fixes the oversized-peg visual AND made the cascade much livelier (10s test: 17→182 hits, 1→5 loops), largely resolving the sparse-cascade tuning item. REMAINING: live input-audio path (RT input ring buffer), draggable drop-point/ball, tempo-sync, delay/reverb type selectors (BACKLOG), board geometry as ValueTree state in presets, denormal/alloc-on-edit hardening. Build: `cmake -B build -S . -G "Visual Studio 17 2022" -A x64; cmake --build build --config Release`.
