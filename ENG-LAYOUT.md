...
- [~] **T1 (P1) — core IN PROGRESS, deterministic foundation DONE (2026-06-17).** Built `core/` (JUCE-free): `PhysicsCore.{h,cpp}` (Box2D v3.1.1, fixed sim step at 1 kHz advanced by a round-to-nearest TIME accumulator → sample-rate-independent + block-size-invariant), `Rng.h` (portable PCG32, no std:: distributions), `tests/determinism_test.cpp`, `CMakeLists.txt` (no -ffast-math/FMA). All tests GREEN on Windows: determinism, block-size invariance, RNG determinism + sensitivity, and liveliness (ball cascades, hits pegs, loops). REMAINING: explicit 44.1/48/96k sample-rate-invariance test; richer board tuning (current cascade is correct but sparse, ~10 hits/loop); wire into the JUCE processor (later task). No-stuck: sustained-slow detection + small mass-scaled horizontal nudge + exit/timeout respawn.
