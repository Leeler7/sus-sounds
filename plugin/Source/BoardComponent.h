...
public:
    ...
    void reloadFromProcessor();           // re-sync the working board from the processor (after a patch load)
...
private:
    ...
    // per-bus brush presets now live in the processor (so the full patch serializes) -- read via proc_
};
