...
- [~] **T7 (P2) — IN PROGRESS (2026-06-17).** Sound engine per AGREED model (ARCHITECTURE §5.2). DONE: `core/SoundEngine.{h,cpp}` (exciter voices + tempo-synced stereo feedback delay + Schroeder reverb), `pegToTap` mapping (x→pan, y→scale pitch, energy→level+brightness), `Wav.h`, `tools/render_demo` (physics→engine→WAV). Peg type plumbed through physics (BoardParams.pegType + Collision.type via shape userData). Renders audible stereo; delay + reverb pegs both fire (demo: 12 delay + 6 reverb), energy/bumper accents work, determinism still green. REMAINING: the input-audio path (currently exciter-only/standalone — the other half of "hybrid"); by-ear feel tuning (cascade still sparse, ~18 hits/12s); FTZ/DAZ denormal guard; then JUCE wiring.
